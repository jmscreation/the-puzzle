...
namespace olc {
    ...
    public:
        ...
        Font(const FT_Byte* data, size_t dataLength, int fontSize) : fontSize(fontSize) {
            FT_Error error = FT_New_Memory_Face(library, data, dataLength, 0, &fontFace);
            if (error) {
                const char *errorString = FT_Error_String(error);
                if (errorString == nullptr) {
                    std::cerr
        ...
        void DrawString(std::string string, int x, int y,
                        olc::Pixel color = olc::BLACK, float angle = 0.0f){
            DrawString(std::wstring_convert< std::codecvt_utf8<char32_t>, char32_t >{}.from_bytes(string), x, y, color, angle);
        }
        ...
        void DrawString(std::string string, olc::vi2d pos,
                        olc::Pixel color = olc::BLACK, float angle = 0.0f){
        ...
        FontRect GetStringBounds(std::string string, float angle = 0.0f) {
            return GetStringBounds(std::wstring_convert< std::codecvt_utf8<char32_t>, char32_t >{}.from_bytes(string), angle);
        }
        ...
        olc::Decal *RenderStringToDecal(std::string string,
                                        olc::Pixel color) {
            return RenderStringToDecal(std::wstring_convert< std::codecvt_utf8<char32_t>, char32_t >{}.from_bytes(string), color);
        }
        ...
        olc::Renderable RenderStringToRenderable(std::string string,
                                                 olc::Pixel color) {
            return RenderStringToRenderable(std::wstring_convert< std::codecvt_utf8<char32_t>, char32_t >{}.from_bytes(string), color);
        }
        ...
        void AddFallbackFont(FT_Byte* data, size_t dataLength) {
            fallbacks.emplace_back(data, dataLength, fontSize);
        }
